(function ($) {
    $(document).on('focus', '.input-emoji', function () {
        ...
        if (input.element.nodeName === 'TEXTAREA') {
        ...
        drop.find('a').on('click', function (event) {
            ...
            if (!document.execCommand('insertText', false, this.innerText)
                && typeof input.element.setRangeText === 'function') {
                var start = input.element.selectionStart;
                input.element.setRangeText(this.innerText);
                // update cursor to be at the end of insertion
                input.element.selectionStart = input.element.selectionEnd = start + this.innerText.length;
            }

...
class Nested extends UcmItem
{
	...
	public function getTree($parentId = null)
	{
		...
		return Service::modelsManager()
			->createBuilder()
			->from(['n' => $thisClass, 'p' => $thisClass])
			->columns('n.*')
			->where('n.lft BETWEEN p.lft AND p.rgt AND n.context = :context: AND p.context = :context: AND n.state = :state: AND p.state = :state: AND p.id = :parentId:', $bindData)
			->orderBy('n.lft')
			->getQuery()
			->execute();
	}

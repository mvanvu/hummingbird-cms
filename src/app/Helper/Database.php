...
class Database
{
	public static function table(string $baseTable)
	{
		$prefix = Service::modelsManager()->getModelPrefix();
		$table  = str_replace('#__', $prefix, $baseTable);
	...
	public static function executeFile($sqlFile)
	{
		if (is_file($sqlFile)
		...
		)
		{
			try
			{
				...
				{
					$db->execute(str_replace('#__', $prefix, $queryString));
				}
			}
			catch (Throwable $e)
			{
				Service::flashSession()->error($e->getMessage());
